...
import { Flame, Drumstick, Droplets, Wheat, List } from "lucide-react";

interface NutritionDisplayProps {
  result: AnalyzeFoodPhotoOutput;
}

const NutritionItem: React.FC<{ icon: React.ElementType; label: string; value: string | number; unit?: string; colorClass?: string }> = ({ icon: Icon, label, value, unit, colorClass = "text-primary" }) => (
...
export default function NutritionDisplay({ result }: NutritionDisplayProps) {
  return (
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          <div className="flex items-center mt-4 mb-2">
            ...
            <h4 className="text-sm font-medium text-foreground">Identified Ingredients:</h4>
          </div>
          {result.ingredients && result.ingredients.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No specific ingredients identified.</p>
          )}
        </div>
      </CardContent>
    </Card>
